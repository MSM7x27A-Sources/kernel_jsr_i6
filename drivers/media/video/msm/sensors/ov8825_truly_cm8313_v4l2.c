...
void ov8825_update_otp_lenc(struct msm_sensor_ctrl_t *s_ctrl,
		uint16_t index)
{
	uint16_t bank, i;
	uint16_t address;
...
//add lens otp
printk("***f:%s,%d***\n",__func__,__LINE__);
	ov8825_update_otp_lenc(s_ctrl, otp_index);
printk("***f:%s,%d***\n",__func__,__LINE__);
	return 0;
}
